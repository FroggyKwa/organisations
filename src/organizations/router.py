...
async def get_organization(organization = Depends(get_organization_by_id)) -> schemas.OrganizationRead:
    return organization


@router.put("/{id}", response_model=schemas.OrganizationRead)
async def update_organization(new_data: schemas.OrganizationUpdate, organization = Depends(get_organization_by_id), db: Session = Depends(get_db)) -> schemas.OrganizationRead:
    return await service.update_organization(db, organization, new_data)


@router.delete("/{id}", status_code=status.HTTP_204_NO_CONTENT)
async def delete_organization(organization = Depends(get_organization_by_id), db: Session = Depends(get_db)) -> None:
